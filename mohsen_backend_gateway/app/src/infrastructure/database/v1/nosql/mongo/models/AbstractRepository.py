...
class AbstractRepository():
    ...
    def insert_many(self, data):
        ...
        try:
            ...
            self._write_error_handler(err)
    def delete(self, id):
        return self.model.update_one({'_id': ObjectId(id)},{'$set': {'is_deleted': True}})

    def updateById(self, id, data):
        data['update_at'] = datetime.now()
        try:
            self.model.update_one({'_id': ObjectId(id),'is_deleted':False}, {'$set': data})
            return str(id)
        except DuplicateKeyError as err:
            self._duplicate_error_handler(err)
        except WriteError as err:
    ...
    def updateByQuery(self, kwargs, data,array_filters=None):
        data['update_at'] = datetime.now()
        kwargs['is_deleted'] = False
        try:
            self.model.update_one(kwargs, {'$set': data},array_filters=array_filters)
    ...
    def join(self,data, is_list = False):
        select = {'is_deleted':0,'created_at':0,'update_at':0,'is_active':0,'_id':0, 'version':0}
